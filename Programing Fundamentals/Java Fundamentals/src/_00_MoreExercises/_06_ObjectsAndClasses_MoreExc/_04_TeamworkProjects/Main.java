package _00_MoreExercises._06_ObjectsAndClasses_MoreExc._04_TeamworkProjects;
...
public class Main {
    public static void main(String[] args) {
        ...
        int countOfTeams = Integer.parseInt(scannner.nextLine());

        List<Team> teamsList = new ArrayList<>();

        while (countOfTeams-- > 0) {
            String[] input = scannner.nextLine().split("-".trim());
            String creator = input[0].trim(); String teamName = input[1].trim();
        ...
        while (!"end of assignment".equals(input = scannner.nextLine())) {
            String[] command = input.split("->".trim());

            String user = command[0].trim(); String teamName = command[1].trim();
